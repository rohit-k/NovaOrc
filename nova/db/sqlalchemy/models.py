...
class ResourceTrackerAction(BASE, NovaBase):
    """Track action perfomed in each chain."""
    __tablename__ = 'resource_tracker_action'
    tracking_id = Column(String(length=255), primary_key=True, nullable=False)
    action_performed = Column(String(length=255), primary_key=True,
                              nullable=False)
    action_result = Column(Text, nullable=False)


class ResourceTracker(BASE, NovaBase):
    """Map resource with tracking id."""
    __tablename__ = 'resource_tracker'
    id = Column(Integer, primary_key=True, nullable=False)
    request_uuid = Column(String(length=255))
    tracking_id = Column(String(length=255), nullable=False)
